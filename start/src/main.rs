fn func_ex_div_some(x: i32, y: i32) -> Option<i32> {  // Option<T> は T 型の値を Some() で包んだものか None のどちらか
    let ans = if y == 0 {  // if 分岐は式なので";"不要
        None
    } else {
        Some(x / y)
    };
...
fn func_ex_div_result(x: i32, y: i32) -> Result<i32, &'static str> {  // エラーハンドリングには Result<T, E> がよく用いられるらしい
    if y == 0 {
        Err("div by zero")
    } else {
        Ok(x / y)
...
fn func_ex_print_some(ans: Option<i32>) -> () {  // 今回の場合は func_ex_div_some の返り値を考えるとこれでいける
    if let Some(x) = ans {
        println!("{}", x)
    } else { // ans が None の場合
        println!("None")
